...
export class FlatplotService {
  ...
  async create(createflatplotDto: CreateFlatPlotdto) {
    const { flatNumber, ownerName, contactNumber, societyId, isrented } = createflatplotDto;
    ...
    await prisma.flatPlot.create({
      data: { flatNumber, ownerName, contactNumber, societyId, isrented },
    });
  ...
  async update(idnumber: number, updateFlatPlotdto: UpdateFlatPlotdto) {
    const { flatNumber, ownerName, contactNumber, societyId, isrented } = updateFlatPlotdto;
